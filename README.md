# MomBestie AI — Marketing Website

Public marketing site for **MomBestie AI**. React 19 + Vite + Tailwind CSS v4.
...
A visitor sees **their own country's language and price by default** — not a dropdown of every locale MomBestie supports. See [`src/i18n.ts`](./src/i18n.ts):

- Country is detected from the browser locale (a sandbox stand-in for real geo-IP — see the file header and mombestie-docs §7.1 for why this needs to become server-side geo-IP before launch).
...
Frontend prototype — no backend. Pricing/commission numbers are reference data (`COUNTRY_PROFILES`), not live config; see mombestie-docs for the plan to source this from a real `country_config` service with human-approved price changes (mombestie-docs §7.2).
